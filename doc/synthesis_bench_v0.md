...
To close `F-RTSC-1` (CSH reproduction failure → next-candidate
...
- F-RTSC-1 retract criterion: 0/τ(6)=0/4 confirmed reproductions
  for any candidate (CSH, Cu-Pb-H, hydride family, infinite-layer
  nickelate) within stated synthesis protocol → spec update
- T3 raw-data evidence required: independent labs (≥ τ=4) reproduce
  candidate Tc + Meissner under `.roadmap §A.4 raw_77_rtsc_csh_repro_v1`
  schema
- σ-φ = 10 false-positive multiplier (numerics_csh anchor) — design
...
|:-:|:-----|:-----|:-------------------|:----------:|:-----|
| 1 | High-pressure cell | 5–20 GPa, ⌀10 mm Belt apparatus | Riken Univ press, Almax | 180k | CSH calls 925°C @ 1 atm; hydride family needs 5–20 GPa |
| 2 | Tube furnace | RT–1500°C, ⌀60 mm bore, 5°C/min ramp | Carbolite STF15/180 | 12k | CSH 925°C heat treatment |
...
| 8 | Inert ball mill | tungsten-carbide vials, 1500 rpm | Retsch PM 100 | 18k | precursor mixing (CSH Lanarkite + Cu₃P route) |
...
|13 | Safety: Pb dust extractor | HEPA H14, exhaust to chimney | Camfil Megalam | 14k | CSH-class hydride synthesis dust hazard |
...
```
   ...
   └────────────┘   └────┬───────────────┘   └────────┬──────────┘
                    ...
                    └─────────────────────────────────────┘
                       ...
                       → schema raw_77_rtsc_csh_repro_v1
...
| XRD diffractogram | DAQ PC | proprietary Bruker | per-scan | XRD pattern matching CSH-class candidate |
...
### §6.1 Pb hazard
- CSH-class hydride (CSH-class candidate) generates Pb dust on grinding/quench.
...
|:-:|:-----------------|:-:|
| T1 algebraic | `verify/calc_csh.hexa` (n=6 closed-form anchors) | ✓ done |
| T2 numerical | `verify/numerics_csh{,_parity,_solver}.hexa` (3-stack) | ✓ done |
| T3 archival  | `verify/empirical_csh_arxiv.hexa` (corpus existence) | ✓ done |
| T3 raw-data  | this bench produces `raw_77_rtsc_csh_repro_v1` data | **TBD** (needs build) |
...
| 4 | First synthesis (CSH baseline batch ×4) | $50k/yr ops | 1 mo |
...
- Cu₃P stoichiometry: x=0.9 vs x=1.1 in carbonaceous sulfur hydride family — affects
  Cu site occupancy.
- 925°C vs 950°C heat-treatment — original CSH group claimed 925°C
...
- `verify/numerics_csh_solver.hexa` (T2 ×3 closed-form solver)
- `verify/empirical_csh_arxiv.hexa` (T3 archival anchor)
